...
class CalendarQuery
{
    ...
    public static function getCalendarViewForGuestQuery($concentrationID, $startDate, $endDate)
    {
        return "SELECT conc.concentration_code, c.course_id, c.course_code, c.course_title, c.attributes,
                ...
                AND c.course_day != ''
                AND DATE(CONCAT(ct.course_year, '-', ct.start_month, '-', ct.start_day)) <= DATE(".CalendarDBUtils::getDBValue(DBConstants::DB_STRING, $startDate).")
                AND DATE(CONCAT(ct.course_year, '-', ct.end_month, '-', ct.end_day)) >= DATE(".CalendarDBUtils::getDBValue(DBConstants::DB_STRING, $endDate).")
