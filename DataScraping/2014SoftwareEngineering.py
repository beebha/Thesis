...
output = open("output/software_engineering_requirements_"+timestamp+".csv", "wb")
writer = UnicodeWriter(output)

# Get the DOM object.
url = URL("http://dceweb.harvard.edu/prod/sswcpgm.taf?function=search&wgrp=ALMIT&_UserReference=E11F5775BEB5C7554DDE88C4&concentrationArea=AREA_CONC_1%2C9&SEARCH_TERM=both")
            ...
            for i in range(len(all_columns)):
                ...
                if i == 0:
                    if ind_column.find("Spring") != -1:
                        term = ind_column + " 2015"
                    elif ind_column.find("Fall") != -1:
                        term = ind_column + " 2014"
                    elif ind_column.find("Summer") != -1:
                        term = ind_column + " 2014"
